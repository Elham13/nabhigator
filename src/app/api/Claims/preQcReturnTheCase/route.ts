...
router.post(async (req) => {
  ...
  try {
    ...
    const emailContent = `<div><p style="font-weight:700">Dear Team,</p><p><span style="font-weight:700">Pre-Auth ID ${
      dashboardData?.claimId
    }</span> is returned from Pre-QC by ${
      user?.name
    }</p><p><span style="font-weight:700">Reason and Remarks: </span>${summaryOfInvestigation}</p><p>Kindly refer to FRCU Final Investigation Report and documents collected, <a href="${webUrl}/pdf-view-and-download?claimId=${getEncryptClaimId(
      dashboardData.claimId
    ...
    const { success, message } = await sendEmail({
      ...
      subject: `Pre-Auth ID ${dashboardData?.claimId}, ${summaryOfInvestigation}`,
      html: emailContent,
    });
